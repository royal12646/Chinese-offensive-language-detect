...
parser.add_argument("--train_path", type=str, default="./Dataset/SexHarmset/SexHarmSet.csv")
parser.add_argument("--model_name", type=str, default="hfl/chinese-macbert-base")
args = parser.parse_args()
df_train = pd.read_csv(args.train_path)  # 微调macbert, 利用dataset里面三个数据集
ds_train = Dataset.from_pandas(df_train)
ds_train = ds_train.shuffle(42)

# 加载模型和tokenizer
model = AutoModelForSequenceClassification.from_pretrained(args.model_name, num_labels=2, use_safetensors=0)
model.to(device)
tokenizer = AutoTokenizer.from_pretrained(args.model_name)  # 需要和模型一致

def tokenize_fn(example):
    label2id = {"Safe": 0, "Harmful": 1}
    result = tokenizer(example["Sentence"], max_length=150, truncation=True, padding="max_length")
    result["labels"] = [label2id[x] for x in example["Type"]]
    return result

ds_encoded = ds_train.map(tokenize_fn, batched=True, remove_columns=ds_train.column_names)  # 支持批处理和多进程map
...
keras_model.fit(train_data=dl_train, val_data=dl_val, ckpt_path='tocp.pth', epochs=5, patience=5, monitor="val_acc",
                mode="max", plot=True, wandb=False)
